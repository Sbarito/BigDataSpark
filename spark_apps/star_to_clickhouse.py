...
# --- PostgreSQL Connection Details (Источник DWH) ---
...
# --- ClickHouse Connection Details (Целевая БД для отчетов) ---
...
def generate_reports(spark):
    ...
    # Средний размер заказа = Общая выручка за месяц / Количество транзакций за месяц
    ...
    # Объем продаж - sum(sale_quantity) для каждого продукта
